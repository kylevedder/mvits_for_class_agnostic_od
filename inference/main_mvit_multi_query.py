...
def parse_arguments():
    ...
    ap.add_argument(
        ...
        default="mdef_detr",
        help=
        "The models to be used for performing inference. Available options are,"
        "['mdef_detr']")
    ap.add_argument(
        "-i",
    ...
    ap.add_argument(
        ...
        default=None,
        help=
        "The path to models checkpoints. Required for all models except mdetr."
    )
    ap.add_argument(
        "-tq_list",
        "--text_queries_list",
        required=False,
        default='[all objects,all entities,'
        'all visible entities and objects,'
        'all obscure entities and objects]',
        help=
        "The list of text queries to be used for generating predictions using MViTs."
    )
    ap.add_argument("-p",
                    "--no_processes",
                    required=False,
    ...
    ap.add_argument("--multi_crop",
                    action='store_true',
                    help="Either to perform multi-crop inference or not. "
...
images_dir = Path(args["input_images_dir_path"])
assert images_dir.is_dir(), 'Input images directory not found'
...
def run_mvit_inference(model_name,
                       checkpoints_path,
                       images_dir,
                       output_path,
                       caption=None,
...
def run(text_queries):
    output_dir = images_dir.parent / model_name
    output_dir.mkdir(exist_ok=True)
    for tq in text_queries:
        print(f"Running query '{tq}'")
        sanitized_tq = tq.replace(" ", "_")
        output_path = output_dir / f'{sanitized_tq}.pkl'
        run_mvit_inference(model_name,
                           checkpoints_path,
                           images_dir,
                           output_path,
                           caption=tq,
